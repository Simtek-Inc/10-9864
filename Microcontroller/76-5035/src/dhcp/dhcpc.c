...
// ----------------------------------------------------------------------------
// Name    : void DHCP_ReceiveHandle(byte XDATA* pData, uint length, byte connId)
// Purpose :
// Params  :
// Returns :
// Note    :
...
void dhcp_Discover(void)
{
	...
	uint			i;
      ...
      // assigned by stack //
	dhcpInfo.SocketId = TCPIP_UdpNew(dhcpInfo.AppId, dhcpConnID, DHCP_CLIENT_BROADCAST, (uint)DHCP_CLIENT_PORT,
					(uint)DHCP_SERVER_PORT);
	...
	TCPIP_UdpSend((dhcpInfo.SocketId), NULL, 0, (byte *)pDiscover, (uint)(DHCP_FIXED_LEN + dhcpInfo.OptionsCounter));
...
void dhcp_Request(void)
{
	...
	TCPIP_UdpSend((dhcpInfo.SocketId), NULL, 0, (byte *)pRequest, (uint)(DHCP_FIXED_LEN + dhcpInfo.OptionsCounter));
...
void DHCP_RejectOffer(void)
{
	...
	TCPIP_UdpSend((dhcpInfo.SocketId), NULL, 0, (byte *)pDecline, (uint)(DHCP_FIXED_LEN + dhcpInfo.OptionsCounter));
	TCPIP_UdpClose(dhcpInfo.SocketId);
}
