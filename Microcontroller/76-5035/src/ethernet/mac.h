...
#define MAC_FORCE_ENBEDDED_PHY_10_HALF	0x4A
	...
	byte	LinkSpeed;
	byte	FullDuplex;
      byte  MediumLinkType;   // map to mac register MAC_MEDIUM_STATUS_MODE_REG //
	byte	NetworkType;
	byte	InterruptMask;
...
{
      byte  Mode;       // see above wakeup frame Mode //
      byte  Offset0;    // filter0 offset //
      byte  Offset1;    // filter1 offset //
      ulong Mask0;      // filter0 mask //
      ulong Mask1;      // filter1 mask //
      byte* Pframe;     // point to the first byte of wakeup frame (destination address) //
	byte	Length;
