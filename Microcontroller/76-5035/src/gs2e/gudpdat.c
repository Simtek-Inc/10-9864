...
//========================================================================================//
// Simtek Revision : A                                                                    //
//    Revision A changed the routine HSUR_GetRxBufCount to GetEthTxBufCount, and          //
//  the routine HSUR_GetCharNb to GetEthChar. This revision was done in an                //
//  attempt to eliminate unsed routines from the program burn file                        //
//                                                                                        //
//========================================================================================//
//========================================================================================//
//========================================================================================//
...
void gudpdat_Client(byte id)                                                              // 
{
      ...
      if (GS2E_GetTaskState() != GS2E_STATE_UDP_DATA_PROCESS)                             // 
        {
        uint   dport = GCONFIG_GetClientDestPort();                                       // get the host server port number
        ...
        if (gudpdat_Conns[id].State == GUDPDAT_STATE_FREE)                                // confirm that the socket is free
          ...
          if(gudpdat_Conns[id].UdpSocket == TCPIP_NO_NEW_CONN)                            // check to make sure the socket was created
            ...
            {
            printd("%du\n\r", (uint) dport);                                              // print the port number
            }          
          }
        }
}
...
// ---------------------------------------------------------------------------------------//
// name    : gudppdat_MaintainConnection()                                                //
...
//                constant definition                                                     //
//========================================================================================//
#define tablesize 2*DimCurveLen	  
      ...
      while(i < length)                                                                   // keep going until the whole ethernet packet is processed
        ...
        switch(EthRxBuf[i])                                                               // check the current byte to see if it is a command byte
          ...
          case ResetRequest : {
                    ...
                    memset(&EthRxBuf[i], 0, 1);                                           // make sure to clear the buffer
                    i += 1;                                                               // update byte index to next location
	      ...
	      case DimmingRequest : {		  			  
                    if(acDimmingRequest < 0xffff)                                         // make sure that the max number of  requests is not pending
                      { acDimmingRequest++; }                                             // trigger a  action in the main loop
                    memcpy(&bufDimmingData[ix02], &EthRxBuf[i+1], 2);
					memset(&EthRxBuf[i], 0, 3);                                                     // clear 3 bytes: 1 command byte + 2 data bytes
                    i += 3;                                                               // update byte index to next location (1 cmd + 2 data bytes)
                    break;
                    }

		  case IndicatorRequest : {
		  			  
                    if(acIndicatorRequest < 0xffff)                                         // make sure that the max number of  requests is not pending
                      { acIndicatorRequest++; }                                             // trigger a  action in the main loop
                    memcpy(&bufIndicatorData[ix02], &EthRxBuf[i+1], 1);
					memset(&EthRxBuf[i], 0, 2);                                                       // make sure to clear the buffer
                    i += 2;                                                                 // update byte index to next location
                    break;
                    }

		  case DisplayRequest : {
		  			  
                    if(acDisplayRequest < 0xffff)                                         // make sure that the max number of  requests is not pending
                      { acDisplayRequest++; }                                             // trigger a  action in the main loop
                    memcpy(&bufDisplayData[ix02], &EthRxBuf[i+1], 6);
					memset(&EthRxBuf[i], 0, 7);                                                     // make sure to clear the buffer
                    i += 7;                                                               // update byte index to next location
                    break;
                    }

		  case DisplayDimmingRequest : {
		  			  
                    if(acDisplayDimmingRequest < 0xffff)                                         // make sure that the max number of  requests is not pending
                      { acDisplayDimmingRequest++; }                                             // trigger a  action in the main loop
                    memcpy(&bufDisplayDimmingData[ix02], &EthRxBuf[i+1], 1);
					memset(&EthRxBuf[i], 0, 2);                                                            // make sure to clear the buffer
                    i += 2;                                                                      // update byte index to next location
                    break;
                    }

		  case DisplayContrastRequest : {
		  			  
                    if(acDisplayContrastRequest < 0xffff)                                         // make sure that the max number of  requests is not pending
                      { acDisplayContrastRequest++; }                                             // trigger a  action in the main loop
                    memcpy(&bufDisplayContrastData[ix02], &EthRxBuf[i+1], 1);
					memset(&EthRxBuf[i], 0, 2);                                                             // make sure to clear the buffer
                    i += 2;                                                                       // update byte index to next location
                    break;
                    }
		  ...
		  case 0xEA         :                                                                 // look for the entire table update command
                    {
                    if(lightingcal == TRUE)                                               // see if the lighitng calibration is on
                      {

                      if(length == (tablesize+1))                                         // make sure that all of the data was sent
                        {
                        printd("\n\rNew Lighting Table Loaded\n\r");                      // let the user know that the command was succesful
                        pword = addresslookup(0);                                         // get the table address
                        memmove(pword, &EthRxBuf[i+1], (tablesize));                      // copy the new data into the table
                        GCONFIG_SetLightingTable(pword);
                        memset(&EthRxBuf[i], 0, ((tablesize)+1));                         // make sure to clear the buffer
                        i += ((tablesize)+1);                                             // update byte index to next location
                        //transmit table back as confirmation
                        }
                      else
                        {
                        memset(&EthRxBuf[i], 0, length);                                  // make sure to clear the buffer
                        i += (length);                                                    // update byte index to next location
                        //transmit fail
                        }
                      }
                    else
                      {
                      printd("no valid op-code : %02bx\n\r",EthRxBuf[i]);                 // let the user know that an invalid operation code was received
                      memset(&EthRxBuf[i], 0, length);                                    // make sure to clear the buffer
                      i += (length);                                                      // update byte index to next location
                      }                   
                    }
                 break;  
            case 0xED         :                                                           // look for the single table entry update command
                    {
                    if(lightingcal == TRUE)                                               // see if the lighitng calibration is on
                      {
                      ...
                      {
                      printd("no valid op-code : %02bx\n\r",EthRxBuf[i]);                 // let the user know that an invalid operation code was received
                      EthRxBuf[i] = 0;                                                    // make sure to clear the buffer after reading
                      i++;                                                                // update byte index to next location
                      }                   
                    }
                 break;
            case 0xEE         :                                                           // look for the sigle table entry update command
                    {
                    if(lightingcal == TRUE)                                               // see if the lighitng calibration is on
                      {
                 ...
                 break;
            case 0xEF         :                                                           // look for the sigle table entry update command
                    {
                    if(lightingcal == TRUE)                                               // see if the lighitng calibration is on
                      {
                      memset(&EthRxBuf[i], 0, length);                                    // make sure to clear the buffer
                      i += (length);                                                      // update byte index to next location
                      acLightTableReadBack = 1;                                           // set an action flag to transmit the lighting table
                      }
                    else
                      {
                      printd("no valid op-code : %02bx\n\r",EthRxBuf[i]);                 // let the user know that an invalid operation code was received
