...
static byte  hsurRxBuffer[UR2_MAX_RX_SIZE];
static byte  hsurTxBuffer[UR2_MAX_TX_SIZE];
static uint  hsurRxBufNum = 0;
static uint  hsurTxBufNum = 0;
static byte  hsurRxTrigLvl = 0;
static uint  hsurRxCount = 0;
static sint  hsurTxCount = 0;
static uint  hsurGetPtr = 0;
static uint  hsurPutPtr = 0;
static byte  hsurFlowCtrl = 0;
static byte  hsurFlowOn = 0;
static byte  hsurTxTransmit = 0;
static byte  hsurTxFlag = 0;
static byte  hsurTxState = 0;
static uint  hsurErrorCount = 0;
static ulong hsurErrTimeStart = 0;
static ulong hsurErrTimeStop = 0;
static byte  hsurErrBlocking = 0;
static uint  hsurBaudRateDivisor = 0;
static byte  hsurLineStatusValue = 0;
static byte  hsurModemStatusValue = 0;     
static byte  hsurLineControlValue = 0;
static byte  hsurModemControlValue = 0;
static byte  hsurFifoControlValue = 0;
static byte  hsurAppFlowOn = 0;
static byte  hsurFlowCtrlXoff = FALSE;
static byte  hsurFlowCtrlXon = FALSE;
static byte  hsurFlowCtrlHwCTSon = FALSE;
static byte  hsurInitIerRec = 0;

#if DEBUG_DISPLAY_DIAG
ulong hsurXonCounter = 0;
ulong hsurXoffCounter = 0;
#endif
...
//----------------------------------------------------------------------------------------
// Name    : HSUR_Setup(uint divisor, byte lCtrl, byte intEnb, byte fCtrl, byte mCtrl)
...
void HSUR_Setup(uint divisor, byte lCtrl, byte intEnb, byte fCtrl, byte mCtrl)
{
      ...
      UR2_DLL = (byte)(divisor & 0x00FF);
      UR2_DLH = (byte)((divisor & 0xFF00) >> 8);
      UR2_LCR &= ~UR2_DLAB_ENB;
      UR2_LCR = lCtrl;
      UR2_FCR = fCtrl;
...
//----------------------------------------------------------------------------------------
// Name    : HSUR_SetFlowControl(byte mode)
...
bool HSUR_SetBaudRate(uint divBaudRate)
{
      byte  recIntrEnb = UR2_IER;

      UR2_IER = 0;
      UR2_LCR |= UR2_DLAB_ENB;
      UR2_DLL = (byte)(divBaudRate & 0x00FF);
      UR2_DLH = (byte)((divBaudRate & 0xFF00) >> 8);
      UR2_LCR &= ~UR2_DLAB_ENB;
...
//----------------------------------------------------------------------------------------
// Name    : HSUR_SetLineControl(byte value)
...
//----------------------------------------------------------------------------------------
// bool HSUR_SetupPort(uint divBaudRate, byte lctrl)
// Purpose : Setting the HSUR baudrate.
// Params  : divBaudRate - divisor of the baudrate.
//           lctrl - line control 
// Returns : 
...
//----------------------------------------------------------------------------------------
// Name    : HSUR_SetFifoControl(byte value)
...
//----------------------------------------------------------------------------------------
// Name    : HSUR_SetModemControl(byte value)
...
//----------------------------------------------------------------------------------------
// Name    : HSUR_GetLineStatus(byte intrSynth)
...
//----------------------------------------------------------------------------------------
// Name    : HSUR_GetModemStatus(byte intrSynth)
