...
byte bufDisplayData[bufDisplayMsgLength];
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~//
	  ...
	  strncpy(&bufDimmingData[ix01],dimmingdefault,sizeof(dimmingdefault));
        ...
        strncpy(&bufDisplayData[ix01],displaydefault,sizeof(displaydefault));
...
void process_FPGA_data(void) 			                                                //
{
	...
	{
		wrHost(&command, 1);
		wrHost(&status_byte1, 1);
		wrHost(&status_byte2, 1);
		wrHost(&status_byte3, 1);
		wrHost(&status_byte4, 1);
		wrHost(&status_byte5, 1);
		wrHost(&status_byte6, 1);
		instant_flag = 0;
		arp_flag = 0;
	}
...
// purpose : Set the LCD and LED brightness based on command data                        //
// params  : void                                                                        //
// returns : void                                                                        //
// notes   : 2nd byte = LCD brightness (0x00=0%, 0x80=50%, 0xFF=100%)                    //
//          3rd byte = LED brightness (same scale)                                       //
//----------------------------------------------------------------------------------------
void processDimmingRsp(void)
{
    char index_lcd;
    ...
    {
        // Byte 2 : LCD brightness
        index_lcd = bufDimmingData[ix02];
        dim_lcd   = xlat2DimCurve(index_lcd);

        // Byte 3 : Indicator brightness
        index_ind = bufDimmingData[ix03];
        dim_ind   = xlat2DimCurve(index_ind);

        // Send LCD dimming
        writeFpgaData(0xC1, dim_lcd, 14);

        // Send Indicator dimming
...
// purpose : Set the indicator status on the instrument                                   //
// params  : void                                                                         //
// returns : void                                                                         //
// notes   :                                                                              //
//----------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------
// name    : processDisplayRsp                                                            //
// purpose : Set the display status on the instrument                                     //
// params  : void                                                                         //
// returns : void                                                                         //
// notes   :                                                                              //
//----------------------------------------------------------------------------------------
void processDisplayRsp(void)
{
    byte dat[6];            // raw display data
    byte formattedData[5];  // data formatted for HI8150
    byte i;

    if(acDisplayRequest > 0x00)
    {
        // Copy 6 bytes from buffer
        for(i = 0; i < 6; i++)
        {
            dat[i] = bufDisplayData[ix02 + i];
        }

        // Format data for HI8150
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~//
      LogicRst                = 1;                                                        // activate FPGA reset
      DELAY_Ms(5);                                                                        // wait for the FPGA to see the reset signal
      LogicRst                = 0;                                                        // deactivate FPGA reset
      acResetRequest          = 1;                                                        //
	  P0 = 0xFF;
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~//      
	  ...
	  //start_up_delay = SWTIMER_Tick;
	  //while( start_up_delay < start_up_delay+3000)
	  //{
	  //	start_up_delay = SWTIMER_Tick;
	  //}
	  acResetRequest          = 1;
