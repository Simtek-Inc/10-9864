...
#define uart_TxChk()              uart1_TxChk()
#define uart_NoBlockGetkey()      uart1_NoBlockGetkey()
#define uart_RxBuf()              uart1_RxBuf()
#define MAX_RX_UART_BUF_SIZE      MAX_RX_UART1_BUF_SIZE
#define uart_DisplayRxPointers(p) uart1_DisplayRxPointers(p)
...
//bool                          UTIL_PortNumToStr(char *hdr, uint *num);
...
//            {"change_password",       NULL,  UTIL_ChangePW,    "return settings to defaults"},
//            {"defaults",              NULL,  UTIL_defaults,    "return settings to defaults"},
...
//            {"uar1_RX_Ptr",           "urp", ComRxPtr,         "display the UART Pointers"},
//            {"uart_RX_Buf",           "urb", ComRxBuf,         "display the UART buffer"},
//            {"slavestatus_1",         "s1",  slavestatus1,     "send status request 1"},
//            {"slavestatus_2",         "s2",  slavestatus2,     "send status request 2"},
//            {"firmware_1",            "f1",  firmware1,        "send firmware request 1"},
//            {"defaults",              NULL,  UTIL_defaults,    "return settings to defaults"},
//            {"firmware_2",            "f2",  firmware2,        "send firmware request 2"},
...
// purpose   : Decide which command was entered, and call command if a match is found
// params    : void *CmdLine
...
//help command
      InBuf     = (char *)CmdLine;                                                        // get the first character entered
      ...
      if(enterpw == TRUE)                                                                 // is the password prompt displayed
        {
        UTIL_Password(CmdLine);                                                           // check entered data for password
        return 0;                                                                         // return a match to calling routine
        }
// scan the various menus for the user command
      for (i = 0; i < MenuNum; i ++)                                                      // menu loop, MenuNum initailized to number of items in the menu during setup
        {
        ptMenuTmp = Menus[i];                                                             // get pointer to next menu list item
        ...
        ptMenuTmp ++;                                                                     // jump past menu ID
                                                                                          // scan this menu's for a valid command word
        while ((ptMenuTmp->Command))                                                      // scan this menu's for a valid command word
          {      
          if((pwcnt == 0) && (ptMenuTmp->Command == "show_device_config"))                // if password is not active end command lookup on item after password
            {    break;        }                                                          // password not active, stop checking menu after "password"
          ...
          if ((Len == 0) || (Len > UTIL_StrLen(InBuf)))                                   // got a match?
            ...
            if (ptMenuCur)                                                                // we already had a match
              {
              printd("1Ambiguous command: %s %du\n\r", InBuf, Len);                                // tell the user of the failure
...
//      printd(" ----------------------------   -------------------------------------------\n\r%-27s\n\r",str);
      printd("\r ----------------------------   -------------------------------------------\n\r");
//      if(pwcnt != 0) pwunlock = 1; else pwunlock = 0;
      while ((ptMenu->Command))// || (pwunlock == 1))
        {
//        if((pwcnt == 0) && (ptMenu->Command == "password")) break;
        ...
        ptMenu ++;
//        if(ptMenu->Command == NULL){ pwunlock = 0; ptMenu++; } 
        }
      printd("\n\r");
}
...
static void UTIL_ReadI2C(void *cmdLine)
{
      ...
      if (EEPROM_Read((byte)(device|0xA0), (byte)addr, &memdata, (uint)len) == I2C_OK)
        ...
        for (i = 0 ; i < len ; i ++)
          {
          if (((i) % 16) == 0)                    printd(" 0x%04dx  |  ",(uint)(addr+i)); // display the address in from of the begining of the line, and then every 16th
...
// clear the buffer
      ...
      if ((char *)cmd[0] == '?')                                                                 // check for the help character
        ...
        len = (end - (uint)start) + 1;                                                    // calculate the requested length
        printd("start     : %02bx\n\r",start);
        printd("end       : %02dx\n\r",end);
        printd("length    : %du\n\r",len);
...
//      printd("%s", (char *)&upgradestring[0]);     
//byte IDATA fwAutoUpdated _at_ 0x31;
//      fwAutoUpdated = Check_Value;// | (~Check_Value << 4);
//const byte                    Check_Value = 0x0a;
//      printd("\n\r");
      while(uart_TxChk());                                                               // make sure all messages have finished transmitting
//      DELAY_Ms(100);
      AX11000_SoftReboot();
}
#endif
...
//          return;    
        ...
        *NextCbuf = 0;                                                                    // null terminate string //
        ...
        printd("\r");
        if(enterpw == TRUE) {                         printd(pwPrompt);    }              // displays the password prompt
//        if(pwcnt > 0) { printd(pwaPrompt); }
        printd(Prompt);
        }
//----------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------
      if ((Key  >= ' ') && (Key < 123))                                                   // got printable char //
        {
//        if(((pwcnt == 0) && (enterpw == FALSE)) && ((Key  >= 'A') && (Key <= 'Z')))       //
        if(((Key  >= 'A') && (Key <= 'Z')))       //
          Key |= 0x20;                                                                    // character an upper case make it lower case
...
//----------------------
// none of the above, check for an <esc> press, escape deletes whole line back to prompt
//----------------------
...
// purpose   : convert uint into ascii string with header
// params    : 
//   input   : pointer to uint array storing the converted bytes of ascii numbera
...
//      dcb->CheckSum = exChecksum((uint *)dcb, sizeof(block));                             //
...
//----------------------------------------------------------------------------------------
// name      : bool UTIL_StrToWord(byte *ConStr, uint *ConRes)
...
static      void              MessageFrames(void)
{
      UTIL_ByteAryToStr("Status Buf",   (byte *)&bufStatusRsp[ix01], bufStatusMsgLength);     //
}
...
static      void              Com1RxBuf(void)
{
      //uart_ShowBuffer(&uart_RxBuf[0], MAX_RX_UART_BUF_SIZE);
}
...
static      void              UTIL_ChangePW(void *cmd)
{
      void        *gpw = NULL;                                                            // given password
      char acceptchar[] = "0123456789abcdefghijklmnopqrstuvwxyABCDEFGHIJKLMNOPQRSTUVWXYZ!@#$%^&*()-_=+[]:;<>/";      // help command line should still be in inbuf //
      int  len;
      int  pos;

      gpw = UTIL_NextArg(cmd);                                                            // get first arg after command

      if (gpw != 0)                                                                       // check to make sure a new password was given
        {
        len = strlen(gpw);                                                                // find the length of the given pawword
        pos = strspn(gpw,acceptchar);                                                     // search given password for unusable chars, pos == lenth if none found
        printd("given password = %s l=%du p=%du\n\r",gpw,len,pos);                        // show user accepted characters for password
        if(len > 10)                                                                      // check for invalid characters in new password
          {
          printd("!Error! : too many characters entered\n\r");                            // show user the problem
          printd("          password can be between 1 and 10 characters\n\r");            // show the string length limits
          printd("          acceptable characters are %s\n\r",&acceptchar);               // show user accepted characters for password
          }
        else if(pos != len)                                                                    // check for invalid characters in new password
          {
          printd("!Error! : invalid characters in new password\n\r");                     // show user the problem
          printd("          password can be between 1 and 10 characters\n\r");            // show the string length limits
          printd("          acceptable characters are %s\n\r",&acceptchar);               // show user accepted characters for password
          }
        else //if (gpw != 0)                                                                // check to make sure a new password was given
          {
//          GCONFIG_SetPassword(gpw);                                                       // set the password to the new password
//          GCONFIG_WriteConfigData();                                                      // store for later
          }
        }
      else                                                                                // no password was given
        {
        printd("!Error! : no new password given.\n\r");                                   // show user the problem
        printd(" Usage  : change_password <new password>\n\r");                           // show the proper usage
        printd("          password can be between 1 and 10 characters\n\r");              // show the string length limits
        printd("          acceptable characters are %s\n\r",&acceptchar);                 // show user accepted characters for password
        }
}
...
static      void              UTIL_Password(void *cmd)
{
      ...
      cmd = cmd;
//      printd("entered password = %s\n\r",cmd);                                            // show the given password
//      if(cmd == 0)                                                                        // make sure that the given password is not a NULL
//        {
//        enterpw = FALSE;                                                                  // the given password is a NULL, set password flags back to defaults
//        pwcnt = 0;                                                                        //
//        return;                                                                           // 
//        }
//	spw = GCONFIG_GetPassword();                                                        // get the stored password
//      err = UTIL_StrCmp(spw, cmd);                                                        // compare the given and stored password
//      if(err == 0)                                                                        // err will be non-zero for no match, or 0 for a match
//        {
//        printd("match\n\r");                                                              // show the user the result
//        pwcnt = pwTimeout;                                                                // set the password enabled timeout
//        }
//      else                                                                                //
        {
        err = strlen(spw);                                                                // find the length of the given pawword
        pos = strspn(spw,acceptchar);                                                     // search given password for unusable chars, pos == lenth if none found
        if((err > 10)||(pos != err))                                                      // check if the length is greater then the allowed password size, or contained any unusable chars
          {
          if(pos != err)                                                                  // check to make sure all characters is stored password were valid
            {
            printd("!Error! : invalid password stored\n\r");                              // let user know about the error, or 
            printd("stored password reset to defualt\n\r");                               //
//            GCONFIG_SetPassword("1234");                                                    // set the password to the default
//            GCONFIG_WriteConfigData();                                                      // write altered DCB to flash
            }
          else                                                                            // there were invalid characters found in the stored password
            {
            printd("all chars found p=%du e%du\n\r",pos,err);                             // show user result of this test
...
static      void              UTIL_LtgCal(void)
{
      if(lightingcal == FALSE)
        {  lightingcal  =  TRUE;   printd("LCAL = ENABLED\n\r");  }
      else
        {  lightingcal  =  FALSE;  printd("LCAL = DISABLED\n\r");  }
}
