/*
 ******************************************************************************
 *     Copyright (c) 2006 	ASIX Electronic Corporation      All rights reserved.
 *
 *     This is unpublished proprietary source code of ASIX Electronic Corporation
 *
 *     The copyright notice above does not evidence any actual or intended
 *     publication of such source code.
 ******************************************************************************
 */
/*=============================================================================
 * Module Name: filesys.h
 * Purpose:
 * Author:
 * Date:
 * Notes:
 * $Log: filesys.h,v $
 *=============================================================================
 */

#ifndef __FILE_SYSTEM_H__
#define __FILE_SYSTEM_H__

/* INCLUDE FILE DECLARATIONS */
#include "types.h"

/* NAMING CONSTANT DECLARATIONS */
#define FILE_ADVSETTING			2
#define FILE_AXJS0			4
#define FILE_BSCSETTING			5
#define FILE_INDEX			6
#define FILE_JMPINDEX			7
#define FILE_MSGACCERR			8
#define FILE_MSGADVERR			9
#define FILE_MSGAUTHERR			10
#define FILE_MSGAUTHOK			11
#define FILE_MSGBSCERR			12
#define FILE_MSGREBOOT			13
#define FILE_MSGRESTORE			14
#define FILE_MSGSCTERR			15
#define FILE_MSGUNPWOK			16
#define FILE_MSGUPG			17
#define FILE_SECURITY			18

#define MAX_STORE_FILE_NUM	19
#define MAX_FILE_NAME_LEN	30

#define FILE_TYPE_HTML		0
#define FILE_TYPE_BIN		BIT0
#define FILE_TYPE_JS		BIT1
#define FILE_TYPE_CSS		BIT1|BIT0

#define MAX_POST_RECORDS	42
#define MAX_POST_BUF_SUBMIT	3
#define MAX_POST_BUF_TEXT	19
#define MAX_POST_BUF_TAG	0
#define MAX_POST_BUF_RADIO	0
#define MAX_POST_BUF_SELECT	14
#define MAX_POST_NAME_LEN	15
#define MAX_POST_VALUE_LEN	65
#define NORM_POST_VALUE_LEN	16

#define MAX_SELECT_OPTION_NUM	9
#define MAX_RADIO_OPTION_NUM	0

#define POST_TYPE_SUBMIT	1
#define POST_TYPE_TEXT		2
#define POST_TYPE_RADIO		3
#define POST_TYPE_SELECT	4
#define POST_TYPE_TAG   	5

#define RECORD_TEXT_tftps_ip		0
#define RECORD_TEXT_file_name		1
#define RECORD_TEXT_email_addr		4
#define RECORD_TEXT_email_from		5
#define RECORD_TEXT_email_to1		6
#define RECORD_TEXT_email_to2		7
#define RECORD_TEXT_email_to3		8
#define RECORD_SELECT_coldstart		9
#define RECORD_SELECT_authfail		10
#define RECORD_SELECT_ipchg		11
#define RECORD_SELECT_pswchg		12
#define RECORD_SELECT_dbr		13
#define RECORD_SELECT_data		14
#define RECORD_SELECT_parity		15
#define RECORD_SELECT_stop		16
#define RECORD_SELECT_flow		17
#define RECORD_SELECT_rs485		18
#define RECORD_SELECT_dhcp		19
#define RECORD_TEXT_static_ip		20
#define RECORD_TEXT_mask		21
#define RECORD_TEXT_gateway_ip		22
#define RECORD_TEXT_dns_ip		23
#define RECORD_SELECT_conntype		24
#define RECORD_TEXT_txtimer		25
#define RECORD_SELECT_cs_mode		26
#define RECORD_TEXT_s_lstport		27
#define RECORD_TEXT_c_deshn		28
#define RECORD_TEXT_c_desport		29
#define RECORD_TEXT_username		30
#define RECORD_TEXT_password		31
#define RECORD_TEXT_new_usn		33
#define RECORD_TEXT_old_psw		34
#define RECORD_TEXT_new_psw		35
#define RECORD_TEXT_cfm_psw		36
#define RECORD_TEXT_acc_ip1		37
#define RECORD_TEXT_acc_ip2		38
#define RECORD_TEXT_acc_ip3		39
#define RECORD_TEXT_acc_ip4		40
#define RECORD_SELECT_acc_ipen		41

/* TYPE DECLARATIONS */
/*-------------------------------------------------------------*/
typedef struct _FILE_MANAGEMEMT
{
	byte			Occupy;
	byte			FType;
	byte			FName[MAX_FILE_NAME_LEN];
	uint			FileSize;
	byte			*PBuf;
	byte			CgiRef;
	void			*CgiCall;

} FILE_MANAGEMEMT;

/*-------------------------------------------------------------*/
typedef struct _POST_RECORD
{
	byte			Occupy;
	byte			PostType;
	byte			Name[MAX_POST_NAME_LEN];
	byte			NameLen;
	byte			FileIndex;
	byte			UpdateSelf;
	void XDATA*		PValue;

} POST_RECORD;

/*-------------------------------------------------------------*/
typedef struct _BUF_SUBMIT
{
	byte			Value[NORM_POST_VALUE_LEN];
	byte			DefaultVlaueLen;
	byte			IsApply;

} BUF_SUBMIT;

/*-------------------------------------------------------------*/
#if (MAX_POST_BUF_RADIO)
typedef struct _BUF_RADIO
{
	uint			Offset[MAX_RADIO_OPTION_NUM];
	byte			Value[MAX_RADIO_OPTION_NUM][NORM_POST_VALUE_LEN];
	byte			Length[MAX_RADIO_OPTION_NUM];
	byte			Count;
	byte			DefaultSet;
	byte			CurrentSet;
	byte			UserSet;

} BUF_RADIO;
#endif

/*-------------------------------------------------------------*/
typedef struct _BUF_TEXT
{
	uint			Offset;
	byte			*CurrValue;
	byte			*UserValue;
	byte			DefaultLength;
	byte			CurrLength;
	byte			UserLength;

} BUF_TEXT;

/*-------------------------------------------------------------*/
typedef struct _BUF_TAG
{
	uint			Offset;
	byte			CurrValue[NORM_POST_VALUE_LEN];
	byte			DefaultLength;
	byte			CurrLength;

} BUF_TAG;

/*-------------------------------------------------------------*/
#if (MAX_POST_BUF_SELECT)
typedef struct _BUF_SELECT
{
	uint			Offset[MAX_SELECT_OPTION_NUM];
	byte			Count;
	byte			DefaultSet;
	byte			CurrentSet;
	byte			UserSet;

} BUF_SELECT;
#endif
/*-------------------------------------------------------------*/
 typedef struct {
	byte* name;
	byte* val;
} entry;
	extern entry entries[20];
	extern byte num_parms;

/*-------------------------------------------------------------*/

/* EXPORTED SUBPROGRAM SPECIFICATIONS */
#if (MAX_STORE_FILE_NUM)
extern FILE_MANAGEMEMT XDATA FSYS_Manage[MAX_STORE_FILE_NUM];
#endif
#if (MAX_POST_RECORDS)
extern POST_RECORD XDATA POST_Record[MAX_POST_RECORDS];
#endif

void FSYS_Init(void);
byte FSYS_FindFile(byte XDATA*);
uint FSYS_HtmlExpansion(byte);

const void *fun_tbl[];

void FUN_TEXT_tftps_ip (void *pWebData);
void FUN_TEXT_file_name (void *pWebData);
void FUN_TEXT_email_addr (void *pWebData);
void FUN_TEXT_email_from (void *pWebData);
void FUN_TEXT_email_to1 (void *pWebData);
void FUN_TEXT_email_to2 (void *pWebData);
void FUN_TEXT_email_to3 (void *pWebData);
void FUN_SELECT_coldstart (void *pWebData);
void FUN_SELECT_authfail (void *pWebData);
void FUN_SELECT_ipchg (void *pWebData);
void FUN_SELECT_pswchg (void *pWebData);
void FUN_SELECT_dbr (void *pWebData);
void FUN_SELECT_data (void *pWebData);
void FUN_SELECT_parity (void *pWebData);
void FUN_SELECT_stop (void *pWebData);
void FUN_SELECT_flow (void *pWebData);
void FUN_SELECT_rs485 (void *pWebData);
void FUN_SELECT_dhcp (void *pWebData);
void FUN_TEXT_static_ip (void *pWebData);
void FUN_TEXT_mask (void *pWebData);
void FUN_TEXT_gateway_ip (void *pWebData);
void FUN_TEXT_dns_ip (void *pWebData);
void FUN_SELECT_conntype (void *pWebData);
void FUN_TEXT_txtimer (void *pWebData);
void FUN_SELECT_cs_mode (void *pWebData);
void FUN_TEXT_s_lstport (void *pWebData);
void FUN_TEXT_c_deshn (void *pWebData);
void FUN_TEXT_c_desport (void *pWebData);
void FUN_TEXT_username (void *pWebData);
void FUN_TEXT_password (void *pWebData);
void FUN_TEXT_new_usn (void *pWebData);
void FUN_TEXT_old_psw (void *pWebData);
void FUN_TEXT_new_psw (void *pWebData);
void FUN_TEXT_cfm_psw (void *pWebData);
void FUN_TEXT_acc_ip1 (void *pWebData);
void FUN_TEXT_acc_ip2 (void *pWebData);
void FUN_TEXT_acc_ip3 (void *pWebData);
void FUN_TEXT_acc_ip4 (void *pWebData);
void FUN_SELECT_acc_ipen (void *pWebData);

#endif /* End of __FILE_SYSTEM_H__ */


/* End of filesys.h */