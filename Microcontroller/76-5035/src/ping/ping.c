...
#define PING_DEFAULT_TIME_OUT 		(175)//Unit in 10ms
...
byte XDATA PingFlag;/* for mwif compatable */
...
static byte PING_ChkQueueEmpty(tsPING_LINKLIST *pFIFO);
...
void PING_Init(void)
{
	memset((byte*)&CmdFIFO, 0, sizeof(tsPING_LINKLIST));
	memset((byte*)&WaitFIFO, 0, sizeof(tsPING_LINKLIST));
	memset((byte*)&ReplyFIFO, 0, sizeof(tsPING_LINKLIST));
...
void PING_Send(tsPING_CONFIG_SET *pEntity)
{
	...
	for (i = 0; i < j; i ++)
	{
		pICMP->Data[i] = (byte)(i + PING_PATTERN_DATA_START);
	}
...
void PING_CallBack(void)
{
	...
	{
		if (pICMP->Data[i] != (byte)(i + PING_PATTERN_DATA_START))
			return;
	}	
 ...
 * ----------------------------------------------------------------------------
 */
byte PING_ChkQueueEmpty(tsPING_LINKLIST *pFIFO)
{
