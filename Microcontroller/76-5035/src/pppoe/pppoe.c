...
byte PPPOE_GetDnsFlag = 0;

/* Pointer of PPPOE_INFO type */
static PPPOE_INFO	pppoe_info;

/* Local static variables */
static byte		ac_cookie[32];
static byte		service_name[32];
static uint	as_length[2];
static byte		host_uniq_from_PADO[8];
static uint	session_id;
static byte		magic_number_server[4];
static byte		magic_number_client[4];
static byte		pppoe_identification_1[11] = "MSRAVS5.10";
static byte		pppoe_identification_2[13] = "MSRAV-0-ASIX";
static byte		pppoe_username[33] = "username"; //User name
static byte		pppoe_password[33] = "password"; //Password
static byte		pppoe_server_ip[4];
static byte		pppoe_ip[4];
static byte		pppoe_pri_dns[4];
static byte		pppoe_sec_dns[4];
static byte		pppoe_successful = 0;
static byte		pppoe_server_mac[6];
static byte		pppoe_MacAddr[6];

/* LOCAL SUBPROGRAM DECLARATIONS */
static void	Analysis_Tags(byte *tags, uint length);
static byte	PPPOE_Discovery_Handle(byte*, byte state);
static byte	PPPOE_Session_Handle(byte*, uint state);
static byte	Check_Magic(byte *magic, byte sc);
static byte	PPPOE_Process(byte *buf, uint length);
static byte	Check_Echo(byte *buf);
static byte	Check_Terminate(byte *buf);
static uint PPPOE_Get_Session(void);
...
byte PPPOE_Receive(byte *buf, uint length)
{
	...
	{
		return PPPOE_Process((byte*)PPPOEBUF, length);
	}
	else
	{
		return 0;
...
void PPPOE_Send(byte* buf, uint length)
{
	...
	*(uint*)PPPOEBUF->payload_data = 0x0021;
}
 ...
 * ----------------------------------------------------------------------------
 */
byte PPPOE_Process(byte *buf, uint length)
{
	pppoe_hdr*	PPPOEBUF = (pppoe_hdr*)buf;
	...
	if(Check_Echo((byte XDATA*)PPPOEBUF))
	{
		return 1;
	}

	/* Check Terminate request */
	if(Check_Terminate((byte XDATA*)PPPOEBUF))
	{
		return 1;
	}
	...
	switch(pppoe_info.state)
	{
		case PPPOE_CODE_PADI:
			...
			if(PPPOEBUF->ether_type == UIP_ETHTYPE_PPPOE_DISCOVERY && PPPOEBUF->pppoe_code == PPPOE_CODE_PADO)
			{
				...
				return PPPOE_Discovery_Handle((byte*)PPPOEBUF, PPPOE_CODE_PADR);
      		}
			return 0;
		case PPPOE_CODE_PADR:
			...
			if(PPPOEBUF->ether_type == UIP_ETHTYPE_PPPOE_DISCOVERY && PPPOEBUF->pppoe_code == PPPOE_CODE_PADS)
			{
				...
				return PPPOE_Discovery_Handle((byte*)PPPOEBUF, PPPOE_CODE_PADS);
			}
			return 0;
		case PPPOE_CODE_PADS:
			...
			{
				return PPPOE_Session_Handle((byte*)PPPOEBUF, 1);
			}
			return 0;
		case PPPOE_CODE_PADS_2:
			...
			if(PPPOEBUF->ether_type == UIP_ETHTYPE_PPPOE_SESSION && PPPOEBUF->session_id == PPPOE_Get_Session())
			{
				return PPPOE_Session_Handle((byte*)PPPOEBUF, 3);
			}
			return 0;
		case PPPOE_CODE_PADS_4:
			...
			if(PPPOEBUF->ether_type == UIP_ETHTYPE_PPPOE_SESSION && PPPOEBUF->session_id == PPPOE_Get_Session())
			{
				return PPPOE_Session_Handle((byte*)PPPOEBUF, 7);
			}
			return 0;
		case PPPOE_CODE_PADS_6:
			...
			if(PPPOEBUF->ether_type == UIP_ETHTYPE_PPPOE_SESSION && PPPOEBUF->session_id == PPPOE_Get_Session())
			{
				return PPPOE_Session_Handle((byte*)PPPOEBUF, 9);
			}
			return 0;
		case PPPOE_CODE_PADS_7:
			if(PPPOEBUF->ether_type == UIP_ETHTYPE_PPPOE_SESSION && PPPOEBUF->session_id == PPPOE_Get_Session())
			{
				return PPPOE_Session_Handle((byte*)PPPOEBUF, 10);
			}
			return 0;
 ...
 * ----------------------------------------------------------------------------
 */
byte PPPOE_Discovery_Handle(byte *PPPOEBUF, byte state)
{ 
	...
	switch(state)
	{
		case PPPOE_CODE_PADI:
			...
			tags->length = 0;
			*(uint*) tags->value = PPPOE_TAG_HOST_UNIQ;
			*(uint*) (tags->value + 2) = 0x0008;
			*(ulong*) (tags->value + 4) = 0x06000000;
			*(ulong*) (tags->value + 8) = 0x0B000000;
		...
		case PPPOE_CODE_PADR:
			...
			Analysis_Tags((byte*)tags, ((pppoe_hdr*)PPPOEBUF)->payload_length);
			...
			((pppoe_hdr*)PPPOEBUF)->session_id = PPPOE_CODE_SESS;
			((pppoe_hdr*)PPPOEBUF)->payload_length = (uint)as_length[0] + (uint)as_length[1] + 20;
			tags->name = PPPOE_TAG_SERVICE_NAME;
			tags->length = as_length[1];
			memcpy(tags->value, service_name, tags->length);
			*(uint*) (tags->value + tags->length) = PPPOE_TAG_HOST_UNIQ;
			*(uint*) (tags->value + tags->length + 2) = 0x0008;
			*(ulong*) (tags->value + tags->length + 4) = 0x06000000;
			*(ulong*) (tags->value + tags->length + 8) = 0x0C000000;
			*(uint*) (tags->value + tags->length + 12) = PPPOE_TAG_AC_COOKIE;
			*(uint*) (tags->value + tags->length + 14) = as_length[0];
		...
		case PPPOE_CODE_PADS:
			...
			return 1;
		default:
			printf("Nothing to do in Discovry of PPPoE !\r\n");
			return 0;
	}
	...
	/* Send packet */
	ETH_SendPkt((byte XDATA*)PPPOEBUF, TCPIP_GetXmitLength());
 ...
 * ----------------------------------------------------------------------------
 */
byte PPPOE_Session_Handle(byte *PPPOEBUF, uint state)
{
	...
	switch(state)
	{
		...
		case 2:
			...
			phs->length = 14;
			*(uint*)phs->option = 0x0104;
			*(uint*)(phs->option + 2) = 0x05c8;
		...
		case 7:
			...
			{
				printf("PPPoE Account or Password not correct!\r\n");
				return 1;
			}
		...
		case 8:
			...
			phs->length = 22;
			((pppoe_hdr*)PPPOEBUF)->payload_length = phs->length + 2;
			*(uint*)(phs->option) = 0x0306;
			*(ulong*)(phs->option + 2) = 0;
			*(uint*)(phs->option + 6) = 0x8106;
			*(ulong*)(phs->option + 8) = 0;
			*(uint*)(phs->option + 12) = 0x8306;
			*(ulong*)(phs->option + 14) = 0;
		...
		case 10:
			...
			if(phs->protocol == IPCP && phs->ppp_code == 0x02 && phs->ppp_id == pppoe_info.identifier)
			{
				...
				STOE_SetIPAddr(*(ulong*)pppoe_ip);
				TCPIP_SetIPAddr(*(ulong*)pppoe_ip);
				...
				printf("Finish the PPP handshake !! %lx\r\n", *(ulong*)pppoe_ip);
				return 1;
			}
			return 0;
		default:
 ...
 * ----------------------------------------------------------------------------
 */
void Analysis_Tags(byte *tags, uint length)
{
	byte	i;
	...
	for(i = 0; i < length; i++)
	{
		...
		{
			as_length[1] = *(uint*)(tags+i+2);
			memcpy(service_name, (tags + i + 4), *(uint*)(tags+i+2));
		}

		/* Host-uniq */
		if((*(tags + i) == 0x01) && (*(tags + i + 1) == 0x03))
		{
			memcpy(host_uniq_from_PADO, (tags + i + 4), *(uint*)(tags+i+2));
		}

		/* AC-Cookie */
		if((*(tags + i) == 0x01) && (*(tags + i + 1) == 0x04))
		{
			as_length[0] = *(uint*)(tags+i+2);
			memcpy(ac_cookie, (tags + i + 4), *(uint*)(tags+i+2));
		}
	}
}
 ...
 * ----------------------------------------------------------------------------
 */
byte Check_Magic(byte *magic, byte sc)
{
	byte	i;
 ...
 * Params: byte *username byte *password
 * Returns:
 * Note: 
 * ----------------------------------------------------------------------------
 */
 ...
 * Params: byte *pri_dns byte *sec_dns
 * Returns:
 * Note: 
 * ----------------------------------------------------------------------------
 */
