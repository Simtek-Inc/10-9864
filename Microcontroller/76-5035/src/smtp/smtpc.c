...
char each3toc;	/* Buffer each 3 byte to coding Base64 format*/
char b64[4];	/* memory output 4 base64 data*/


/* STATIC VARIABLE DECLARATIONS */
static byte *smtpc_Buf;
static byte *smtpc_Subject;
static SMTPC_CONN XDATA smtpc_Conns;
static byte XDATA smtpc_InterAppId;
 ...
 * ----------------------------------------------------------------------------
 */
void SMTPC_Init(byte *pBuf, byte *pSubject)
{
 ...
 * ----------------------------------------------------------------------------
 */
void SMTPC_Event(byte id, byte event)
{
	if (id != 0)
		return;
 ...
 * ----------------------------------------------------------------------------
 */
void SMTPC_Receive(byte XDATA* pbuf, uint length, byte id)
{
	uint		codes, len;
	byte		*point, *pData;
 ...
 * ----------------------------------------------------------------------------
 */
void SMTPC_SendMessage(byte* pBuf, uint length)
{
	uint		len, j;
	byte		*point, *pData, *ptemp;
	char*		pimage;		/* point to image data */
	byte		char_count, i;
	byte		multi = 0;
 ...
 * ----------------------------------------------------------------------------
 */
void SMTPC_Start(ulong ip, byte* from, byte* to1, byte* to2, byte* to3)
{
