...
static byte XDATA pingSequence[2] = {0, 0};
static uint ip_Id = 0;
...
void PING_Init(void)
{
	byte	i = sizeof (PingConnect);
	byte*	point = (byte*)&PingConnect;
...
void PING_ReplyCheck(void)
{
	...
	if ((pIpHeader->SourIp == PingConnect.IP) &&
		(*(uint XDATA*)pIcmpHeader->Sequence == PingConnect.Sequence))
	{
...
void ping_Send(void)
{
	...
	PingConnect.Sequence = *(uint*)pingSequence;
